...
void NavMeshManager::enqueueJob(NavArea* area, AABB areaToBuild, const RecastSettings& recastConfig, const String& queue) {
	...
	Core::getTaskManager()->scheduleTask([=] { checkJobs(); }, "checkJobs", 750, TileQueue.toCharArray());
}
...
void NavMeshManager::startJob(Reference<NavMeshJob*> job) {
	...
	info() << "Starting building navmesh for area: " << name << " on planet: " << zone->getZoneName() << " at: " << area->getPosition().toString();
	...
	Core::getTaskManager()->executeTask(
		[area, name, builder, initialBuild, this] {
			if (stopped)
				return;
			...
			info() << "Done building and setting navmesh for area: " << name << " on planet: " << zone->getZoneName() << " at: " << area->getPosition().toString();
		},
		"setNavMeshLambda");
	...
	// TODO: Fixme
	Core::getTaskManager()->scheduleTask([=] { checkJobs(); }, "checkNavJobs", 1000, TileQueue.toCharArray());
}
