...
void ObjectVersionUpdateManager::updateWeaponsDots() {
	...
	try {
		while (iterator.getNextKeyAndValue(objectID, &objectData)) {
			...
			try {
				if (!Serializable::getVariable<String>(classNameHashCode, &className, &objectData) || !Serializable::getVariable<int>(STRING_HASHCODE("WeaponObject.dotType"), &oldType, &objectData)) {
					objectData.clear();
					continue;
				}
			} catch (...) {
...
void ObjectVersionUpdateManager::updateTangibleObjectsVersion6() {
	...
	while (iter.hasNext()) {
		...
		if (tmpl->isSharedTangibleObjectTemplate()) {
			...
			if ((tanotmp->getUseCount() > 0) || (lootSchem != nullptr && lootSchem->getTargetUseCount() > 1) || tmpl->getGameObjectType() == SceneObjectType::FACTORYCRATE) {
			...
			for (int i = 0; i < subGroups->size(); i++) {
				...
				if (subGroupTitle == "usecount" || subGroupTitle == "quantity" || subGroupTitle == "charges" || subGroupTitle == "uses" || subGroupTitle == "charge") {
					templateKeys.put(key);
					info("Adding Tangible Template: " + tanotmp->getTemplateFileName(), true);
					continue;
				}
	...
	try {
		while (iterator.getNextKeyAndValue(objectID, &objectData)) {
			...
			try {
				if (!Serializable::getVariable<int>(STRING_HASHCODE("TangibleObject.useCount"), &useCount, &objectData) || !Serializable::getVariable<uint32>(STRING_HASHCODE("SceneObject.serverObjectCRC"), &objCRC, &objectData)) {
					objectData.clear();
					continue;
				}
			} catch (Exception& e) {
...
void ObjectVersionUpdateManager::updateResidences() {
	...
	try {
		while (iterator.getNextKeyAndValue(objectID, &objectData)) {
			...
			try {
				if (!Serializable::getVariable<String>(STRING_HASHCODE("_className"), &className, &objectData) || !Serializable::getVariable<uint64>(STRING_HASHCODE("PlayerObject.declaredResidence"), &residence, &objectData)) {
					objectData.clear();
					continue;
				}
			} catch (...) {
			...
			if (className == "PlayerObject") {
				...
				if (Serializable::getVariable<SortedVector<unsigned long long>>(STRING_HASHCODE("PlayerObject.ownedStructures"), &structureList, &objectData) && Serializable::getVariable<uint64>(STRING_HASHCODE("PlayerObject.declaredResidence"), &residence, &objectData)) {
