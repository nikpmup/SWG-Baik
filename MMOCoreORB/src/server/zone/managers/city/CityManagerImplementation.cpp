...
void CityManagerImplementation::sendCityReport(CreatureObject* creature, const String& planetName, byte rank) {
	...
	for (int i = 0; i < cities.size(); ++i) {
		...
		report << ", " << String::valueOf(city->getObjectID()) << ", " << String::valueOf(city->getMayorID());
		...
		report << ", " << String::valueOf(city->getRegionsCount()) << ", " << String::valueOf(city->getCitizenCount()) << ", " << String::valueOf(city->getStructuresCount()) << ", " << String::valueOf(city->getAllStructuresCount()) << ", " << String::valueOf((int)city->getCityTreasury())
			   << ", x: " << String::valueOf(city->getPositionX()) << " y:" << String::valueOf(city->getPositionY()) << ", " << city->getNextUpdateTime()->getFormattedTime();
...
void CityManagerImplementation::sendStructureReport(CityRegion* city, CreatureObject* creature, SceneObject* terminal) {
	...
	for (int i = 0; i < city->getStructuresCount(); i++) {
		...
		if (structure != nullptr)
			maintList->addMenuItem(structure->getObjectName()->getFullPath() + " - Condition : " + String::valueOf((1.0f * structure->getMaxCondition() - structure->getConditionDamage()) / structure->getMaxCondition() * 100) + "%", i);
	}
	...
	for (int i = 0; i < city->getDecorationCount(); i++) {
		...
		if (deco != nullptr)
			maintList->addMenuItem(deco->getObjectName()->getFullPath() + " - Condition : " + String::valueOf((1.0f * deco->getMaxCondition() - deco->getConditionDamage()) / deco->getMaxCondition() * 100) + "%", i);
	}
...
void CityManagerImplementation::registerCitizen(CityRegion* city, CreatureObject* creature) {
	...
	if (mayor != nullptr && mayor->isPlayerCreature()) {
		...
		chatManager->sendMail("@city/city:new_city_from", "@city/city:new_city_citizen_subject", params, // City Growth: Added Citizen
		...
		chatManager->sendMail("@city/city:new_city_from", "@city/city:new_city_citizen_other_subject", params, // Welcome, Citizen!
...
void CityManagerImplementation::sendAddStructureMails(CityRegion* city, StructureObject* structure) {
	...
	if (mayor != nullptr && mayor->isPlayerCreature() && owner != nullptr && owner->isPlayerCreature()) {
		...
		chatManager->sendMail("@city/city:new_city_from", "@city/city:new_city_structure_subject", params, // City Growth: Added Structure
							  mayorCreature->getFirstName(), nullptr);
		...
		chatManager->sendMail("@city/city:new_city_from", "@city/city:new_city_structure_other_subject", params2, // Structure Added to City
