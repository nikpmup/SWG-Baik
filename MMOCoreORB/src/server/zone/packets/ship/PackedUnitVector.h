...
template <class T>
inline const T clamp(const T& minT, const T& t, const T& maxT) {
	return (t < minT) ? minT : ((t > maxT) ? maxT : t);
}

class PackedUnitVector {
	enum { packedSize = 2, xSignMask = 0x8000u, ySignMask = 0x4000u, zSignMask = 0x2000u, componentMask = 0x003fu, componentMaskBits = 6 };
