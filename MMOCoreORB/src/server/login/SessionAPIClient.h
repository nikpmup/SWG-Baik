...
public:
	enum ApprovalAction { UNKNOWN = -2, TEMPFAIL = -1, ALLOW = 0, WARN = 1, REJECT = 2, BAN = 3, DEBUG = 4 };
...
public:
	...
	void approvePlayerConnect(const String& ip, uint32 accountID, uint64_t characterID, const ArrayList<uint32>& loggedInAccounts, const SessionAPICallback& resultCallback);
